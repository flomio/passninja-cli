
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === \"object\" && typeof Reflect.decorate === \"function\") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === \"object\" && typeof Reflect.metadata === \"function\") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { \"default\": mod };
};
Object.defineProperty(exports, \"__esModule\", { value: true });
var event_bus_1 = __webpack_require__(/*! ../event-bus */ \"./src/services/event-bus.ts\");
var events_1 = __webpack_require__(/*! ../../events */ \"./src/events.ts\");
var request_promise_native_1 = __importDefault(__webpack_require__(/*! request-promise-native */ \"./node_modules/request-promise-native/lib/rp.js\"));
var injection_js_1 = __webpack_require__(/*! injection-js */ \"./node_modules/injection-js/index.js\");
var logging_1 = __webpack_require__(/*! ../../logging */ \"./src/logging.ts\");
var injection_tokens_1 = __webpack_require__(/*! ../../injection-tokens */ \"./src/injection-tokens.ts\");
var HttpReportsService = /** @class */ (function () {
    function HttpReportsService(events, config) {
        var _this = this;
        this.events = events;
        this.config = config;
        var reportEndPoint = config.args.scanReportEndPoint;
        if (reportEndPoint) {
            var scanEvents = [
                // TODO: just use `scan`
                events_1.Events.smartTapData,
                events_1.Events.vasData
            ];
            scanEvents.forEach(function (ev) {
                _this.events.on(ev, function (data) {
                    request_promise_native_1.default({
                        method: 'POST',
                        url: reportEndPoint,
                        body: JSON.stringify(data)
                    }).catch(function (err) {
                        var firstLine = err.message.split('\
')[0];
                        logging_1.dbg('Failed to post to', reportEndPoint, firstLine);
                    });
                });
            });
        }
    }
    HttpReportsService = __decorate([
        injection_js_1.Injectable(),
        __param(1, injection_js_1.Inject(injection_tokens_1.CONFIG_TOKEN)),
        __metadata(\"design:paramtypes\", [event_bus_1.EventBus, Object])
    ], HttpReportsService);
    return HttpReportsService;
}());
exports.HttpReportsService = HttpReportsService;


//# sourceURL=webpack://commonjs/./src/services/local-reports/http-reports.service.ts?"