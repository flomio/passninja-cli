...
import { SecureSmartTapSession } from "smart-tap"
...
export class LocalSessionHandlerService /** extends AbstractSessionHandler */ {
  ...
  getDecrypter = () => {
    ...
    const decrypter = apple_vas_data_decrypt_1.makeDecrypter(
  ...
  handleMessage = async message => {
    ...
    if (!(message.cmd === messages_1.CommandKey.select_ose)) {
      return
    }
    ...
    const parsed = session.parseSelectOSEResponse(
      session_utils_1.fromBase64(message.args.response)
    )
    ...
    switch (_b.label) {
      case 2:
        ...
        getCommand = await session.getSmartTapDataCommand()
        ...
        return [
          2 /*return*/,
          {
            ...
            args: {
              negotiate: session_utils_1.toBase64(negotiateCommand),
              get: session_utils_1.toBase64(getCommand)
            }
          }
      ...
      case 6:
        return [3 /*break*/, 14]
      case 7:
        if (!(message.cmd === messages_1.CommandKey.decrypt_smart_tap_data)) {
          return [3 /*break*/, 13]
        }
        session = message.session

        message.args.responses.forEach(response => {
          apdu = session_utils_1.fromBase64(response)

          logging_1.trc(
            "parsed get st data resp apdu",
            apdu.slice(-2),
            apdu.length
          )
          await session.parseGetSmartTapDataResponse(apdu)
        })

      case 12: // & case 11:
        fullPayload = session.parseFullPayload()
        values = !!fullPayload
          ? smart_tap_1.getRedemptionValues(fullPayload)
          : []
        return [
          2 /*return*/,
          {
...
//           return [
//             4 /*yield*/,
//             session.parseSelectOSEResponse(
//               session_utils_1.fromBase64(message.args.response)
//             )
//           ]
//         case 2:
...
//           return [3 /*break*/, 8]
//         case 11:
//           return [4 /*yield*/, session.parseFullPayload()]
//         case 12:
//           fullPayload = _b.sent()
//           values = !!fullPayload
//             ? smart_tap_1.getRedemptionValues(fullPayload)
...
//   var decrypter = apple_vas_data_decrypt_1.makeDecrypter(
//     this.config.nfc.selectPassTypeIdentifier,
//     key1.privateKeyPem
//   )
//   return decrypter
